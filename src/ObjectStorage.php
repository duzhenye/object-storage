...
class ObjectStorage
{
     ...
     * @throws \Duzhenye\ObjectStorage\ErrorException
     * @throws \Duzhenye\ObjectStorage\ObjectLockedException
     */
     ...
     * @throws \Duzhenye\ObjectStorage\ErrorException
     * @throws \Duzhenye\ObjectStorage\ObjectLockedException
     * @throws \Duzhenye\ObjectStorage\ObjectNotFoundException
     */
    ...
    public function execute(array $commands): array
    {
        ...
        $prepareFileForWriting = function ($filename) use (&$filePointers, &$emptyOpenedFiles, $logStorageAccess) {
            ...
            if (is_dir($filename)) {
                throw new \Duzhenye\ObjectStorage\ErrorException('The file ' . $filename . ' is not writable (dir with the same name exists).');
            }
            if ($this->createFileDirIfNotExists($filename) === false) {
                throw new \Duzhenye\ObjectStorage\ErrorException('The file ' . $filename . ' is not writable (cannot create dir).');
            }
            ...
            if (!$done) {
                throw new \Duzhenye\ObjectStorage\ObjectLockedException('The file ' . $filename . ' is locked and cannot be open for writing.');
            }
        };
        ...
        $prepareFileForReading = function ($filename, $required = false) use (&$filePointers, $logStorageAccess) {
            ...
            if (is_file($filename)) {
                ...
                if (!is_readable($filename)) {
                    throw new \Duzhenye\ObjectStorage\ErrorException('The file ' . $filename . ' is not readable.');
                }
            } else {
                if ($required) {
                    throw new \Duzhenye\ObjectStorage\ObjectNotFoundException('The file ' . $filename . ' does not exist.');
                }
                ...
                if (!$isParentDirReadable) {
                    throw new \Duzhenye\ObjectStorage\ErrorException('The file ' . $filename . ' is not readable (parent dir is not readable).');
                }
            }
        };
        ...
        set_error_handler(function ($errno, $errstr, $errfile, $errline) {
            ...
            throw new \Duzhenye\ObjectStorage\ErrorException($errstr, 0, $errno, $errfile, $errline);
        });
                ...
                if ($command === 'set') {
                    ...
                    $functions[$index] = function () use ($sourceKey, $targetKey, $getFileContent, $setFileContent, $deleteFile) {
                        ...
                        if ($sourceBody === null) { // The source file is deleted in previous command
                            throw new \Duzhenye\ObjectStorage\ObjectNotFoundException('The source object (' . $sourceKey . ') does not exists!');
                        } else {
